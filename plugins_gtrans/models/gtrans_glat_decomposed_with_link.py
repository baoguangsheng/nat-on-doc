...
from ..modules.nonautoregressive_gtransformer import NAGTransformerDecoder, NAGTransformerModel, tokens2tags
...
@register_model("gtrans_glat_decomposed_link")
class GTransGlatDecomposedLink(NAGTransformerModel):
    ...
    def restore_valid_links(self, links, output_tokens):
        ...
        res = res[:, :, :prelen]
        # Guangsheng Bao: restrict the links inside each sentence
        tags = tokens2tags(self.tgt_dict, output_tokens)
        res_mask = tags.unsqueeze(2) != tags.unsqueeze(1)
        res.masked_fill_(res_mask, float("-inf"))
        return res
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        if self.args.max_transition_length != -1:
            links = self.restore_valid_links(links, output_tokens)
        ...
        output_masks = output_tokens.eq(self.unk) & unreduced_tokens.ne(self.bos) & unreduced_tokens.ne(self.eos)
        unreduced_tokens = unreduced_tokens * output_masks + output_tokens * (~output_masks)
        ...
        if self.args.decode_strategy in ["lookahead", "greedy"]:
            ...
            for i, length in enumerate(output_length):
                ...
                res = [last]
                while j != length - 1:
                    # Guangsheng Bao: start another sentence after the end of a sentence
                    if last == self.tgt_dict.eos_index:
                        j = j + 1
                    else:
                        j = links_idx[i][j]
...
class GlatLinkDecoder(NAGTransformerDecoder):
...
@register_model_architecture(
    "gtrans_glat_decomposed_link", "gtrans_glat_decomposed_link_6e6d512"
)
...
@register_model_architecture(
    "gtrans_glat_decomposed_link", "gtrans_glat_decomposed_link_base"
)
