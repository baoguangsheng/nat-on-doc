...
run_path=$exp_path/run-nat-ctc-default
mkdir -p $run_path
...
if [ $mode == "train" ]; then
  ...
  python train.py $bin_path --arch nat_ctc --noise full_mask --share-all-embeddings --seed 0 --fp16 \
      ...
      --max-source-positions 512 --max-target-positions 1024 --clip-norm 2   \
      --save-dir $cp_path --length-loss-factor 0 --log-interval 1000 \
