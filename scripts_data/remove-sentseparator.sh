...
# remove sentence separator <s> and </s>
for D in test valid train; do
  echo `date`, $D
  cat $src_seg_path/$D.$slang | sed -e 's/ <\/s> <s> / /g' | sed -e 's/<s> //g' > $seg_path/$D.$slang
  cat $src_seg_path/$D.$tlang | sed -e 's/ <\/s> <s> / /g' | sed -e 's/<s> //g' > $seg_path/$D.$tlang
done
